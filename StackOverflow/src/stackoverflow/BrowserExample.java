...
public class BrowserExample {
	static ResourceBundle resourceBundle = ResourceBundle.getBundle("stackoverflow");
	int index;
	boolean busy;
	Image images[];
	Image icon = null;
	...
	static final String[] imageLocations = {"sample.gif"};
	static final String iconLocation = "sample.gif";
		
	//cambiado de Composite a TabItem
	public BrowserExample(TabItem item, boolean top) {
		final Composite parent = new Composite(item.getParent(), SWT.NONE);
		item.setText("SOBrowser");
		...
		browser.addOpenWindowListener(new OpenWindowListener() {
			public void open(WindowEvent event) {
				...
				shell.setLayout(new FormLayout());  //AQUI SE LO PONE COMO FILLLAYOUT!! --> FORMLAYOUT
				BrowserExample app = new BrowserExample(item2, false); //MODIFICADO DE SHELL A ITEM
				app.setShellDecoration(icon, true);
				event.browser = app.getBrowser();
			}
		});
		if (top) {
			browser.setUrl("http://stackoverflow.com/users/login");
			show(false, null, null, false, true, true, true); //no addressbar
		} else {
	...
	void show(boolean owned, Point location, Point size, boolean addressBar, boolean menuBar, boolean statusBar, boolean toolBar) {
		...
		if (toolBar) {
			...
			//FORZAMOS QUE A LA DER DE PRUEBAB (FUTURO MENU, POR EJ)
			//data.left = new FormAttachment(pruebaB, 0);
			...
			final ToolItem itemQ = new ToolItem(toolbar,SWT.PUSH);
			itemQ.setText("Ask");
			final ToolItem itemTagsLcel = new ToolItem(toolbar,SWT.PUSH);
			itemTagsLcel.setText("#lcel");
			...
			Listener listener = new Listener() {
				public void handleEvent(Event event) {
					...
					else if (item == itemQ) browser.setUrl("http://stackoverflow.com/questions/ask");
					else if (item == itemTagsLcel) browser.setUrl("http://stackoverflow.com/questions/tagged/lcel");
			...
			if(addressBar){itemGo.addListener(SWT.Selection, listener);}
			itemQ.addListener(SWT.Selection, listener);
			itemTagsLcel.addListener(SWT.Selection, listener);
			...
			canvas.addListener(SWT.MouseDown, new Listener() {
				public void handleEvent(Event e) {
					browser.setUrl("http://stackoverflow.com");  //CAMBIO DE PAGINA PRINCIPAL
				}
			});
	...
	public static void main(String [] args) {
		...
		BrowserExample app = new BrowserExample(item, true);
		app.setShellDecoration(icon, true);
		shell.open();
